  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-40 bg-white/70 backdrop-blur-md border-b border-gray-100/50 shadow-sm transition-all duration-500">
      <ul className="flex justify-center gap-6 md:gap-12 py-4 text-[10px] md:text-xs tracking-widest uppercase text-gray-600 font-medium">
        ...
        <li><a href="#team" onClick={(e) => handleScroll(e, 'team')} className="hover:text-primary transition-colors">Team</a></li>
...
const FAQs = () => (
  <motion.section
    ...
    className="py-6 md:py-24 max-w-3xl mx-auto px-4"
  >
    ...
    <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex gap-4 md:gap-6">
          {siteConfig.dressCode.colors.map(c => (
            <div key={c.name} className="flex flex-col items-center gap-2">
              {/* Note: I slightly reduced the circle sizes here so they fit perfectly inside a list */}
      ...
      {siteConfig.faqs.map((faq, i) => (
        <motion.div
          ...
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          // Notice we added + 1 to the delay so it perfectly follows the Dress Code animation
