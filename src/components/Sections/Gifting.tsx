...
export default function GiftingModal({ isOpen, onClose }: GiftingModalProps) {
    ...
    const normalizeKenyanMobile = (value: string) => {
        ...
        else if (digits.startsWith("2540") && digits.length === 13) digits = `254${digits.slice(4)}`;
        return digits;
    };
    ...
    const handleSTKPush = async (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        if (!isValidKenyanMobile(formattedPhone)) {
            setPhoneError("Enter a valid Safaricom mobile number.");
            return;
        }

        setStatus("loading"); setErrorMessage(""); setPhoneError("");
                        ...
                        <div className="p-8 overflow-y-auto">
                            <AnimatePresence mode="wait">
                                {status === "idle" && (
                                    <motion.div key="idle" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6 text-center">
                                        ...
                                        <form onSubmit={handleSTKPush} className="space-y-5 text-left">
                                            ...
                                            <button type="submit" className="w-full mt-6 py-4 bg-[#52B44B] text-white rounded-xl tracking-widest uppercase text-sm font-medium hover:bg-[#43963e] transition-colors shadow-lg shadow-green-500/30">Send via STK Push</button>
                                        </form>
                                ...
                                {status === "waiting_for_pin" && (
                                    <motion.div key="waiting" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center text-center space-y-6 py-8"><div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center relative"><div className="absolute inset-0 border-2 border-[#52B44B] rounded-full animate-ping opacity-20" /><svg className="w-10 h-10 text-[#52B44B]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg></div><div><h4 className="text-xl font-serif text-gray-900 mb-2">Check Your Phone</h4><p className="text-gray-500 font-light text-sm px-4">An M-Pesa prompt has been sent to your phone. Please enter your PIN to send the gift.</p></div><p className="text-xs text-primary/60 italic animate-pulse">Waiting for confirmation...</p><button onClick={resetForm} className="text-xs text-gray-400 uppercase tracking-widest mt-8 hover:text-gray-800 transition-colors">Cancel</button></motion.div>
                                )}
                                {status === "success" && (
                                    <motion.div key="success" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center text-center py-8"><div className="w-20 h-20 bg-green-100 text-[#52B44B] rounded-full flex items-center justify-center mb-6"><svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div><h4 className="text-3xl font-serif text-gray-900 mb-2">Thank You!</h4><p className="text-gray-600 font-light mb-6">Your generous gift has been received.</p><div className="bg-gray-50 px-6 py-4 rounded-xl border border-gray-100 w-full"><p className="text-xs text-gray-400 uppercase tracking-widest mb-1">Receipt Number</p><p className="font-mono text-xl text-gray-800 tracking-wider font-bold">{receipt}</p></div><button onClick={resetForm} className="mt-8 text-sm text-primary uppercase tracking-widest hover:underline">Send another gift</button></motion.div>
                                )}
                                {status === "error" && (
                                    <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center text-center py-8"><div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mb-6"><svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></div><h4 className="text-xl font-serif text-gray-900 mb-2">Transaction Failed</h4><p className="text-gray-500 font-light text-sm px-4 mb-8">{errorMessage}</p><button onClick={resetForm} className="px-8 py-3 bg-gray-900 text-white rounded-full uppercase tracking-widest text-xs hover:bg-gray-800 transition-colors">Try Again</button></motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                    </motion.div>
